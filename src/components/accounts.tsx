...
const AccountSkeleton = () => (
  <div className="space-y-4">
    <div className="flex flex-row justify-between items-center p-3 border border-white/20 rounded-lg">
      <div>
      ...
      {userAccounts && userAccounts.length > 0 ? (
        userAccounts.map((account) => (
          <Link href={`/account/${account.id}`} key={account.id}>
            <div className="flex flex-row justify-between items-center p-3 border border-white/20 rounded-lg cursor-pointer hover:bg-white/20">
              <div>
