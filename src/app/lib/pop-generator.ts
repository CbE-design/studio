...
export async function generateProofOfPaymentPdf(transaction: Transaction, account: Account): Promise<Uint8Array> {
    ...
    page.drawLine({ start: { x: margin, y: y + 8 }, end: { x: width - margin, y: y + 8 }, thickness: 0.75, color: rgb(0, 0, 0) });
    ...
    const drawDetailRow = (label: string, value: string) => {
        ...
        page.drawText(':', { x: margin + 130, y, font, size: 9, color: textColor });
        page.drawText(value, { x: margin + 140, y, font, size: 9, color: textColor });
        y -= 15;
    };
    ...
    const wrapText = (text: string, maxWidth: number, textFont: PDFFont, fontSize: number) => {
        ...
        for (let i = 1; i < words.length; i++) {
            const word = words[i];
            if (!word) continue;
            const w = textFont.widthOfTextAtSize(currentLine + " " + word, fontSize);
            if (w < maxWidth) {
                currentLine += " " + word;
            } else {
    ...
    const commonTextOptions = { font, size: 8, color: textColor, lineHeight: 11, maxWidth: width - margin * 2 };
