...
import { ArrowLeft, Mail, MessageSquare, Send, LoaderCircle, X } from 'lucide-react';
...
function ShareProofOfPaymentContent() {
  ...
  const { user } = useUser();
  const { firestore } = useFirebase(); // Assuming you have functions initialized here
  ...
  const handleAction = async (actionId: string) => {
    if (isGenerating || isSendingEmail) return;
    ...
    try {
        ...
        toast({ title: "PDF Ready", description: "Your proof of payment has been downloaded." });
    } catch (error: any) {
  ...
  const handleSendEmail = async () => {
    if (!emailRecipient) {
        toast({ variant: 'destructive', title: 'Error', description: 'Please enter a recipient email address.' });
        return;
    }
    ...
    try {
        ...
        // This is a placeholder call. You'll need to set up the 'sendEmail' function.
        const sendEmail = httpsCallable(firestore as any, 'sendEmail');
        
        await sendEmail({
            to: emailRecipient,
            subject: 'Proof of Payment from VAN SCHALKWYK FAMILY TRUST',
            html: `<p>Dear Recipient,</p><p>Please find attached the proof of payment for ${formatCurrency(Number(paymentDetails.amount))}.</p><p>Regards,<br/>VAN SCHALKWYK FAMILY TRUST</p>`,
            attachments: [
                {
        ...
        toast({ title: "Email Sent", description: "The proof of payment has been sent." });
        setShowEmailDialog(false);
        setEmailRecipient('');

    } catch (error: any) {
        console.error("Failed to send email:", error);
        toast({ variant: 'destructive', title: 'Email Sending Failed', description: 'Could not send email. Please check the function logs.' });
    } finally {
  ...
  return (
    <>
          ...
          <div className="bg-white rounded-lg shadow-sm border">
            {shareOptions.map((option, index) => (
              <div
                ...
                className={`flex items-center p-4 cursor-pointer hover:bg-gray-50 ${index < shareOptions.length - 1 ? 'border-b' : ''} ${(isGenerating || isSendingEmail) ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                {(isGenerating && option.id !== 'email') || (isSendingEmail && option.id === 'email') ?
      ...
      <Dialog open={showEmailDialog} onOpenChange={setShowEmailDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Send Proof of Payment via Email</DialogTitle>
            <DialogDescription>
              Enter the recipient's email address below. The PDF will be generated and attached automatically.
            </DialogDescription>
          </DialogHeader>
